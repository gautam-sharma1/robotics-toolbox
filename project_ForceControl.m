%Force_Control_compliance indirect_Kp and Kd only
function [xr_history] = project_ForceControl(q0,qd0,a,alpha,d,m_li,m_mo,I_li,I_mo,qlim,kr,G,N,tspan,Kp,Kd,x_d,h_e,frame,RP)
...
f_jac = MyJacobian(robot,q_i);
f_jac = f_jac(1:3,:);
f_jac_t = MyJacobian_Transpose(robot,q_i);
f_jac_t = f_jac_t(:,1:3);
xf = f_jac' * inv(K_P) * f_jac_t' * h_e;
xf = [xf;0];
